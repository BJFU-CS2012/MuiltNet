...
class NewExchange(nn.Module):
    ...
    def forward(self, features, bns, bn_threshold):
        ...
        bn1_idx_big2small = bn1.sort(0, True)[1].cpu().numpy().tolist()  # importance ranking(Time-domain)
        bn2_idx_big2small = bn2.sort(0, True)[1].cpu().numpy().tolist()  # importance ranking (Frequency-domain)
