...
class CANet(nn.Module):
    def __init__(self, config):
        ...
        if self.config.model_name == 'resnet50':
            ...
            self.backbone.conv_block4 = nn.Sequential(
                ...
                BasicConv(self.feature_size, 64, kernel_size=3, stride=1, padding=1, relu=True),
                nn.AdaptiveMaxPool2d(1),
            )
    ...
    def forward_vanilla(self, input):
        ...
        # print('---->f1.shape',f1.shape) #torch.Size([32, 512, 44, 44])
        # print('---->f2.shape',f2.shape) #torch.Size([32, 1024, 22, 22])
        # print('---->f3.shape',f3.shape) #torch.Size([32, 2048, 11, 11])
        feats = f3
        ...
        # print('====>f1.shape',f1.shape) #torch.Size([32, 64, 44, 44])
        # print('====>f2.shape',f2.shape) # torch.Size([32, 64, 22, 22])
        # print('====>f3.shape',f3.shape) # torch.Size([32, 64, 11, 11])
        ...
        # print('feat_DCT2.shape',feat_DCT2.shape) # torch.Size([32, 64, 88, 88])
        # print('feat_DCT3.shape',feat_DCT3.shape) # torch.Size([32, 64, 44, 44])
        # print('feat_DCT4.shape',feat_DCT4.shape) # torch.Size([32, 64, 22, 22])
        # print('feat_DCT5.shape',feat_DCT5.shape) # torch.Size([32, 64, 11, 11])
        #feature fusion
        ...
        # print('feat2.shape',feat2.shape) # torch.Size([32, 64, 88, 88])
        # print('feat3.shape',feat3.shape) # torch.Size([32, 64, 44, 44])
        # print('feat4.shape',feat4.shape) # torch.Size([32, 64, 22, 22])
        # print('feat5.shape',feat5.shape) # torch.Size([32, 64, 11, 11])
        ...
        # output_1 = self.conv_out(feat3) # torch.Size([32, 1, 88, 88])
        # output_2 = self.conv_out(feat4) # torch.Size([32, 1, 44, 44])
        # output_3 = self.conv_out(feat5) # torch.Size([32, 1, 22, 22])
        ...
        # -------------------------------------------------------------------------------------------------
        # print('ccc=========================ccc')
        f11 = self.backbone.conv_block4(feat3).view(-1, self.num_ftrs // 2)
        f22 = self.backbone.conv_block4(feat4).view(-1, self.num_ftrs // 2)
        f33 = self.backbone.conv_block4(feat5).view(-1, self.num_ftrs // 2)
        f33_b = self.backbone.b3(f33)
        output = self.backbone.fc(f33_b)
        # print('f11.shape',f11.shape)
        # print('f22.shape',f22.shape)
